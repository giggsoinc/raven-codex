...
_SAFE_COMPLEX_DEFAULT = "openai/gpt-4o"
_EMBEDDING_MODEL_RE = re.compile(r"(embed|minilm|bge-|e5-|reranker)", re.IGNORECASE)
_PREMIUM_MODEL_RE = re.compile(r"(?:^|/)(o1|o3|opus|gpt-4\.5|gpt-5-pro)(?:$|[-:])", re.IGNORECASE)
...
def _load_model_env() -> Dict[str, str]:
    ...
    defaults = {
        ...
        "MEDIUM": "openai/gpt-4o",
        "COMPLEX": _SAFE_COMPLEX_DEFAULT,
        "LOCAL_ONLY": "ollama/dolphin-mistral",
    }

    if not model_env_path.exists():
        return sanitize_routing(defaults)
    ...
    return sanitize_routing(models)
...
def is_chat_capable_model_name(model: str) -> bool:
    """Reject model names that are clearly embeddings/rerankers."""
    return not bool(_EMBEDDING_MODEL_RE.search(model or ""))


def is_premium_model_name(model: str) -> bool:
    """True for models Raven must never auto-select silently."""
    return bool(_PREMIUM_MODEL_RE.search(model or ""))


def premium_opt_in_enabled() -> bool:
    """Explicit opt-in gate for premium model auto-selection."""
    return os.environ.get("RAVEN_ALLOW_PREMIUM_MODEL", "").lower() in {"1", "true", "yes"}


def safe_model_for_tier(tier: str, model: str, allow_premium: bool = False) -> str:
    """Return a safe routing model for a tier, replacing invalid choices."""
    if tier == "LOCAL_ONLY":
        return model if _is_local_model(model) and is_chat_capable_model_name(model) else _DEFAULT_LOCAL
    if not is_chat_capable_model_name(model):
        return "openai/gpt-4o-mini" if tier == "SIMPLE" else _SAFE_COMPLEX_DEFAULT
    if is_premium_model_name(model) and not allow_premium:
        return _SAFE_COMPLEX_DEFAULT
    return model


def sanitize_routing(models: Dict[str, str], allow_premium: bool | None = None) -> Dict[str, str]:
    """Return routing with non-chat and non-opted-in premium models replaced."""
    allow = premium_opt_in_enabled() if allow_premium is None else allow_premium
    return {tier: safe_model_for_tier(tier, model, allow) for tier, model in models.items()}


def validate_routing(models: Dict[str, str], allow_premium: bool | None = None) -> Tuple[bool, List[str]]:
    """Validate routing is chat-capable and honest about premium model use."""
    allow = premium_opt_in_enabled() if allow_premium is None else allow_premium
    problems: List[str] = []
    for tier in ("LOCAL_ONLY", "SIMPLE", "MEDIUM", "COMPLEX"):
        ...
        if not model:
            ...
            continue
        if not is_chat_capable_model_name(model):
            problems.append(f"{tier}: {model} is not a chat-generation model")
        if tier == "LOCAL_ONLY" and not _is_local_model(model):
            problems.append(f"{tier}: {model} is not local")
        if tier != "LOCAL_ONLY" and is_premium_model_name(model) and not allow:
